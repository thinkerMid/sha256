class SHA256:
    ...
    def Compress(self, Wt, Kt, A, B, C, D, E, F, G, H):
        tempA = (self.rightrotate(E, 6) ^ self.rightrotate(E, 11) ^ self.rightrotate(E, 25))
        tempB = (self.rightrotate(A, 2) ^ self.rightrotate(A, 13) ^ self.rightrotate(A, 22))
        tempC = ((A & B) ^ (A & C) ^ (B & C))
        tempD = ((E & F) ^ (~E & G))
        tempE = (D + (H + tempA + tempD + Wt + Kt)) & ((2**32)-1)
        tempF = ((H +tempA +((E & F) ^ (~E & G)) + Wt + Kt) + tempB + tempC) & ((2**32)-1)
        tempG = (H +tempA +((E & F) ^ (~E & G)))
        tempH = (H +tempA +((E & F) ^ (~E & G)) +Kt)
        tempJ = (H +tempA +((E & F) ^ (~E & G))+Wt+ Kt)
        tempK = tempB+tempC # (self.rightrotate(A, 2) ^ self.rightrotate(A, 13) ^ self.rightrotate(A, 22)) + ((A & B) ^ (A & C) ^ (B & C))
        return ((H +tempA +((E & F) ^ (~E & G))+Wt+ Kt) + tempB + tempC) & ((2**32)-1), \
               A, \
               B, \
               C, \
               (tempE) & ((2**32)-1), \
    ...
    def hash(self, message):
        ...
        for i in range(0, len(message), 64):
            ...
            for j in range(64):
                print("A:{:#x} B:{:#x} C:{:#x} D:{:#x} E:{:#x} F:{:#x} G:{:#x} H:{:#x}".format(A,B,C,D,E,F,G,H))
